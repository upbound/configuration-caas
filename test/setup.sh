...
EOF
  ...
  if [[ -n "${UPBOUND:-}" ]]; then
    echo "Creating the upbound-saas default credentials secret..."
    ${KUBECTL} -n upbound-system create secret generic upbound-creds --from-literal=credentials="${UPBOUND}" --dry-run=client -o yaml | ${KUBECTL} apply -f -

    echo "Creating the upbound-saas default provider config..."
    cat <<EOF | ${KUBECTL} apply -f -
